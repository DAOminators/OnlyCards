...
#var is_in_bottom_panel = true  # Tracks the card's current panel
#
#func _on_Card_pressed():
	#if is_in_bottom_panel:
		#move_to_top_panel()
	#else:
		#move_to_bottom_panel()
#
#func move_to_top_panel():
	#var top_container = get_tree().get_root().find_node("$Panel4/HBoxContainer", true)
	#if top_container:
		#top_container.add_child(self)
		#is_in_bottom_panel = false
#
#func move_to_bottom_panel():
	#var bottom_container = get_tree().get_root().find_node("$Panel2/HBoxContainer", true)
	#if bottom_container:
		#bottom_container.add_child(self)
